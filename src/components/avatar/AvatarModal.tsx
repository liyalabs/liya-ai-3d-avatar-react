...
export default function AvatarModal({
  ...
  const [currentVisemes, setCurrentVisemes] = useState<Array<{ time: number; viseme: number; duration: number }>>([])
  ...
  useEffect(() => {
    ...
    if (isProcessing) {
      ...
      timer = setInterval(() => {
        ...
        if (elapsed > 8000) {
          setPreparingMessageIndex(prev => (prev + 1) % t.preparingMessages.length)
        }
      }, 4000)
    } else {
      setPreparingMessageIndex(0)
  ...
  }, [isListening, isProcessing, preparingMessageIndex, t.voice.listening, t.preparingMessages, t.voice.speakToMic])

  const assistantDisplayName = assistantName || config.assistantName || 'AI Assistant'
  ...
  const handleSendMessage = async (message: string) => {
    ...
    } catch (error) { /* fail silent */ } finally {
  ...
  const playAudioWithSync = async (base64Audio: string) => {
    try {
      ...
      const audioBuffer = await new Promise<AudioBuffer>((resolve, reject) => {
        ctx.decodeAudioData(arrayBuffer, resolve, (err) => reject(err || new Error('Decode failed')))
      })

      if (audioSourceRef.current) {
        audioSourceRef.current.stop(); audioSourceRef.current.disconnect()
      }
      ...
      audioSourceRef.current.onended = () => {
        setIsSpeaking(false); setAudioCurrentTime(0); setCurrentVisemes([])
  ...
  const simulateSpeaking = (text: string) => {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      visemes.push({ time, viseme, duration: 0.05 }); time += 0.05
    }
    setCurrentVisemes(visemes); setIsSpeaking(true); setAudioCurrentTime(0)
    const startMs = Date.now()
    const anim = () => {
      const elapsed = (Date.now() - startMs) / 1000
      setAudioCurrentTime(elapsed)
      if (elapsed < duration) requestAnimationFrame(anim)
      else { setIsSpeaking(false); setCurrentVisemes([]) }
    }
    anim()
  }
  ...
  const speakWithAvatar = async (text: string) => {
    try {
      const apiUrl = config.apiUrl || ''; const apiKey = config.apiKey || ''
      const response = await fetch(`${apiUrl}/api/v1/external/avatar/speech/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': apiKey },
        body: JSON.stringify({ text, voice: 'nova', speed: 1.0, include_audio: true })
      })
      if (!response.ok) throw new Error('Speech API failed')
      const data = await response.json()
      if (data.status === 'success' && data.data) {
        setCurrentVisemes(data.data.visemes || [])
        if (data.data.audio_base64) await playAudioWithSync(data.data.audio_base64)
      }
    } catch (e) { simulateSpeaking(text) }
  }
  ...
  const handleClose = () => {
    if (audioSourceRef.current) {
      audioSourceRef.current.stop(); audioSourceRef.current.disconnect(); audioSourceRef.current = null
    }
    setIsSpeaking(false); onClose()
  }
  ...
  return createPortal(
    <div className={`liya-ai-3d-avatar-react-avatar-modal-overlay ${isOpen ? 'liya-ai-3d-avatar-react-avatar-modal-overlay--open' : ''}`} onClick={(e) => e.target === e.currentTarget && handleClose()}>
      <div className="liya-ai-3d-avatar-react-avatar-modal">
        <div className="liya-ai-3d-avatar-react-avatar-modal__header">
          <div className="liya-ai-3d-avatar-react-avatar-modal__title">
            <div className={`liya-ai-3d-avatar-react-avatar-modal__status ${isSpeaking ? 'liya-ai-3d-avatar-react-avatar-modal__status--speaking' : ''}`} />
            <span>{assistantDisplayName}</span>
          </div>
          <button className="liya-ai-3d-avatar-react-avatar-modal__close" onClick={handleClose} aria-label={t.kiosk.close}>
        ...
        <div className="liya-ai-3d-avatar-react-avatar-modal__message">
          {isProcessing ? (
            ...
            <p className="liya-ai-3d-avatar-react-avatar-modal__welcome">{welcomeMessage}</p>
          ) : (
        ...
        <div className="liya-ai-3d-avatar-react-avatar-modal__controls">
          {isVoiceSupported && (
            ...
            >
              {!isListening ? (
                <svg viewBox="0 0 24 24" fill="currentColor" width="32" height="32"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm-1-9c0-.55.45-1 1-1s1 .45 1 1v6c0 .55-.45 1-1 1s-1-.45-1-1V5zm6 6c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>
              ) : (
                <svg viewBox="0 0 24 24" fill="currentColor" width="32" height="32"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>
              )}
            </button>
          )}
          <p className="liya-ai-3d-avatar-react-avatar-modal__hint">{hintText}</p>
        </div>
      </div>
    </div>,
    document.body
